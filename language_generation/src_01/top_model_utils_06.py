...
class Top_model():
    ...
    def get_probs(self, ids, pos_tags=None):
        """get next word probability distributions"""
        mask = torch.ones(ids.shape).int()
        pos_embeds = self.model.pos_embedding(pos_tags) if pos_tags is not None else None
        with torch.no_grad():
            outputs = self.model(input_ids=ids.to(self.device), attention_mask=mask.to(self.device), pos_embeds=pos_embeds)
    ...
    def get_probs_generation(self, content_all, additional_bs, additional_bs_mask, content_prev_sep):
        ...
        content_all, content_all_mask = self.prompt_model.tokenize(content_all, content_all_mask, additional_bs[:content_all.shape[0]], additional_bs_mask[:content_all.shape[0]], content_prev_sep[:content_all.shape[0]], use_fake=False, mode='test')
...
class LanguageModel():
    ...
    def ps(self, contexts, additional_bs=None, additional_bs_mask=None, content_prev_sep=None, pos_tags=None):
        ...
        if additional_bs is None:
            probs = self.model.get_probs(context_arr, pos_tags=pos_tags)
        else:
            probs = self.model.get_probs_generation(context_arr, additional_bs=additional_bs, content_prev_sep=content_prev_sep, additional_bs_mask=additional_bs_mask, pos_tags=pos_tags)
        return probs[:, -1]
...
def generate_beam(model, tokenizer, beam_size: int = 5, embed=None, pos_tags=None, entry_length=32, temperature=1., stop_token: str = '.', bad_words_ids = None, context_ids = []):
    model.eval()
    ...
    with torch.no_grad():
        generated = embed
        pos_tags = pos_tags.expand(beam_size, *pos_tags.shape[1:]) if pos_tags is not None else None
        for i in range(entry_length):
            if pos_tags is not None:
                outputs = model(inputs_embeds=generated, pos_tags=pos_tags)
            else:
            ...
            if scores is None:
                ...
                if pos_tags is not None:
                    pos_tags = pos_tags[next_tokens_source]
